...
- 2026-02-20 - revalidation policy preference - User prefers pausing mutation-triggered public revalidation for now and keeping a documented queued/debounced design before re-enabling.
- 2026-02-20 - invalidation scope preference - Do not use `revalidatePath` with dynamic route patterns for public pages; invalidate concrete paths only.
- 2026-02-20 - sitemap invalidation preference - Do not invalidate `/sitemap.xml` from dashboard mutation hooks.
- 2026-02-20 - reviewer follow-up - Keep invalidation controls in code (not env), separate broad vs cultivar-link toggles, and avoid AdminMenu cache-metadata double fetch by loading once on open state change.
- 2026-02-20 - toggle simplification - User prefers one code toggle (`REVALIDATION_ENABLED`) for all mutation-triggered revalidation behavior instead of split flags.
