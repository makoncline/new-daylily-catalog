...
- 2026-02-24 - dashboard breadcrumbs navigation - `src/components/breadcrumbs.tsx` must render `next/link` (via `BreadcrumbLink asChild`) for dashboard breadcrumb items; plain `href` on shadcn `BreadcrumbLink` causes hard navigations.
...
- 2026-02-24 - list/manage navigate-save test semantics - Validate route-unmount auto-save with in-app navigation (`Link`/sidebar/row action), not `page.goto(...)`; full document navigations can abort pending tRPC writes and produce false failures.
- 2026-02-24 - manage-list external dirty tracking - On `/dashboard/lists/[listId]`, instant list-membership writes (add/remove listing) should flip an external-unsaved flag consumed by `ListForm.hasPendingChanges()` so close/navigate boundaries still call `list.update` as the single future invalidation point.
