...
- [2026-04-27] Dashboard full bootstrap must split Vercel work: A single listing-anchored `dashboardDb.bootstrap.snapshot` can exceed Vercel's 10s function limit for the largest catalog even after local query optimization. Cold bootstrap and warm full refresh should fetch the optimized collection sync routes as separate unbatched requests, then apply one combined local snapshot; add route-specific cursor/chunking only if an individual collection route still times out.
