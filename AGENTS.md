...
- [2026-07-05] Cloudflare SWR proof: Cloudflare treats `s-maxage` as blocking async stale-while-revalidate, returning `EXPIRED` instead of `UPDATING`. When testing public HTML SWR, use a scoped Cache Response Rule to cloudflare-only remove `s-maxage`, set `max-age`, and set `stale-while-revalidate`; prove success with `MISS -> HIT -> UPDATING -> HIT`.
