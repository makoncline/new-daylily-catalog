...
- [2026-07-13] Navigation save guards: A pushState sentinel used to block dirty browser navigation truncates valid Forward history, and `preventDefault()` cannot cancel traversal. For the Chrome-based listings workspace prototype, hold interceptable same-document traversal with a Navigation API `precommitHandler`; keep cross-document and older-browser traversal best-effort until broader blocking is a product requirement.
- [2026-07-13] Spawned pnpm termination needs child forwarding: Sending SIGTERM to a programmatically spawned `pnpm` command can terminate pnpm and its direct lifecycle process while leaving that process's spawned Next server alive. When a local launcher spawns Next and is itself managed by another script, forward SIGINT/SIGTERM to the Next child so agent loops do not leave a dev lock or port behind.
