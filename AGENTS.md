...
- [2026-04-10] ChatGPT agent-browser sessions trigger captchas: On this Mac, a headed isolated `agent-browser` session against chatgpt.com is detected as a test browser and falls into repeated captchas before reaching the project workflow. When automating the local ChatGPT image queue here, do not pivot to `agent-browser`; use a real user browser/profile session instead.
- [2026-04-10] ChatGPT worker now expects a dedicated Chrome user-data dir: The tracked image worker in `scripts/image-processing/v2-ahs-image-review/chatgpt-worker.mjs` points CDP at `local/v2-ahs-image-review/chrome-user-data/DevToolsActivePort`, not the normal system Chrome or Brave profile. When resuming that workflow, launch Chrome with `scripts/image-processing/v2-ahs-image-review/launch-chatgpt-chrome.sh` first and keep Brave for normal browsing.
...
- [2026-04-10] ChatGPT worker should stop on Chrome approval prompts: If `chrome-cdp` returns `Daemon failed to start — did you click Allow in Chrome?`, the local ChatGPT image worker should not keep failing queue items. In this repo it now notifies, puts the current item back to `pending`, and exits so the operator can click Allow and restart cleanly.
