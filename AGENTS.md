...
- [2026-07-14] PostHog surveys are outside Radix dialogs: Popover surveys mount an open shadow root on a `document.body` host whose class is `PostHogSurvey-{surveyId}`. Radix treats survey pointer/focus events as outside interactions and can dismiss an underlying edit dialog; shared dialog content should prevent outside dismissal when the original event's composed path contains that host.
