...
- [2026-04-08] Turso large SQL import transport limit: Streaming the full generated V2 cultivar upsert SQL (~59 MB) through `turso db shell < ...` failed repeatedly with `stream not found` on this machine and Turso setup. For this repo, generate the V2 import as chunk files under `prisma/data-migrations/20260407_upsert_v2_ahs_cultivars/` and apply them with `pnpm db:apply:v2-ahs-cultivar-import -- --db <name>` instead of sending one giant SQL file.
...
- [2026-04-08] V2 display source must be exclusive: When `NEXT_PUBLIC_USE_V2_CULTIVAR_DISPLAY_DATA` is enabled and a `CultivarReference.v2AhsCultivar` exists, the app-facing AHS-shaped display payload must come entirely from the mapped V2 row, not a merge of V2 plus legacy `AhsListing` fields. Fall back to legacy only when no V2 row is available, and do not carry legacy AHS ids through the display payload.
