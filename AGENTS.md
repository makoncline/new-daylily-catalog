...
- [2026-07-20] Prod-like Clerk personas are environment-specific: `prodlike+clerk_test_rollingoaks@example.com` and `prodlike+clerk_test_plantfancy@example.com` with code `424242` are guaranteed by the sanitized realistic-data/stage-Clerk workflow, not by the production-Clerk Docker smoke. The production Clerk sign-in on `dev.daylilycatalog.com` currently reports both accounts missing; do not assume these credentials can authenticate the prod-like container, and remap only the disposable DB clone if a production-Clerk test user is separately provisioned.
