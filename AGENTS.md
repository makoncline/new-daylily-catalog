...
- [2026-07-17] Atlas seed existence is not freshness: `run-atlas-flow.mjs` only prepares the realistic database when the file is missing, so an existing seed from an older schema can pass startup and then fail dashboard bootstrap with missing-column errors. Before member Atlas captures after schema changes, run `pnpm db:seed:prepare` even when `apps/main/local/realistic-data/realistic-data.sqlite` already exists.
