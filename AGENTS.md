...
- [2026-07-14] PostHog rage-click enrichment ordering: `posthog-js` 1.352 emits `$rageclick` synchronously from its click autocapture handler and classifies it as safe to amend through `before_send`. When attaching tap-phase diagnostics, register capture-phase listeners before `posthog.init` so the final click is buffered, and preserve all standard event properties.
