...
- [2026-07-12] Anonymous onboarding real-product boundary: The conversion flow now searches the real cultivar registry through the bounded public `onboarding.searchCultivars` procedure, but selected cultivar data and listing edits stay in the versioned browser draft and are never imported as real listings. When changing onboarding, preserve the real-data/browser-write split and reuse production listing presentation instead of adding simplified fake product screens.

- [2026-07-12] Public catalog search tour reuse seam: Basic public catalog search is the global listing query plus live For Sale, Has Photo, and Lists filters; Advanced mode adds the registry sections for listing text/price, cultivar registration, bloom traits, and classification details. When demonstrating search in onboarding, reuse `public-catalog-search-registry.ts`, the production column filter functions, result counts, and filter chips with a deterministic demo dataset instead of implementing a second search algorithm.
- [2026-07-12] Anonymous onboarding analytics identity seam: The current anonymous browser journey and server-side `onboarding_email_collected` event cannot form a normal PostHog person funnel because the server event uses `draftId` while the browser uses PostHog's anonymous distinct ID. When instrumenting pre-auth onboarding, initialize PostHog before the first step and pass the browser distinct ID to validated server capture calls, with `draft_id` retained as a diagnostic event property.
- [2026-07-12] Anonymous listing preview is deliberately disposable: The pre-checkout onboarding claim imports only profile data; the example listing title, price, description, and image remain local and are discarded. When refining this flow, do not ask prospects to perform detailed fake-listing work; keep it an explicit lightweight demonstration and measure the first real dashboard listing as activation.
