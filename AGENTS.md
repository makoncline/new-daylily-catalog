...
- [2026-06-12] V2 refresh artifacts are per-run temp folders: Ongoing AHS V2 refresh snapshots, generated SQL, review HTML, and scratch verification files should live under ignored `temp/v2-ahs-refresh-runs/<run-id>/`. When running future refreshes, set `RUN_DIR` and pass `TEMP_DIR`, `DB_FILE`, `--source-db`, `--output-dir`, and review-page `--output` into that folder instead of reusing root `cultivars.db`, `temp_pages/`, `downloads/v2-ahs-refresh-review/`, or `prisma/data-migrations/v2-ahs-cultivar-delta/`.
- [2026-06-12] V2 delta generator runs with Node type stripping: This checkout does not currently have a local `tsx` binary, and `npx tsx` tries to hit the network. Run `scripts/generate-v2-ahs-cultivar-delta-sql.ts` with `node --experimental-strip-types` unless a local TypeScript runner is added.
- [2026-05-16] Clawpatch needs unsandboxed Codex state: `pnpm dlx clawpatch review` shells out to the local Codex CLI and fails in the default sandbox because Codex writes `~/.codex/state_5.sqlite`. When trying Clawpatch here, use a fresh worktree from `origin/main` and run the review command with escalated permissions; keep `.clawpatch/` as local review state unless intentionally committing a report.
- [2026-04-27] S3 image backup manifest is SQLite: The image backup workflow records S3 upload state in `downloads/v2-ahs-image-review/s3-manifest.sqlite`, not per-run JSONL files. Future production-import work should read `s3_image_backup_manifest` for `CultivarReference.id`, V2 AHS id, normalized name, local path, S3 key, sha256, bytes, and status instead of scanning/parsing S3 object names.
- [2026-04-27] Image backup skips generated variant relics: Local edited images under `downloads/v2-ahs-image-review/edited/` are meant to be one output per cultivar reference. If a file has a suffix like `1000_1.png`, treat it as a script-testing relic/variant and skip it for S3 backup rather than creating a `variant-1` object. The intended S3 key shape is `cultivar-images/{normalized-name}-{CultivarReference.id}.png`.
- [2026-04-27] V2 source images can be fake JPEG 404 bodies: Some daylilies.org V2 `image_url` assets return `200` plus `content-type: image/jpeg` but only contain 8 bytes of text like `404.html` (for example Winter Rainbow id 66236). Before uploading originals to ChatGPT, validate image magic bytes and mark invalid sources as `source_invalid` instead of waiting on a disabled send button.
- [2026-06-12] V2 refreshes require a human review gate: The repeatable V2 AHS refresh process should stop after generating SQL/review artifacts and before any Turso apply. When future agents run this workflow, produce a change report, let the human ask about exact row/field changes and corrections, then rehearse and apply only the explicitly approved batch.
- [2026-04-28] ChatGPT project generation may stay on the project URL: The ChatGPT project UI can now render sent prompts and generated images inline while `location.href` remains `/project` instead of changing to `/c/<conversationId>`. In `scripts/image-processing/v2-ahs-image-review/chatgpt-worker-agent-browser.mjs`, do not require a conversation URL before saving; snapshot existing generated image URLs before submit and wait for a new generated image.
...
- [2026-04-27] ChatGPT image worker queue priority: In `scripts/image-processing/v2-ahs-image-review/chatgpt-worker-agent-browser.mjs`, the worker claims listing-linked cultivar references first. Once none remain, it falls back to all remaining eligible queued images ordered by `V2AhsCultivar.introduction_date` descending, so newer registrations like 2026/2025 run before older cultivars.
...
- [2026-06-12] Codex-native image subagents need file existence checks: When using Codex-native image generation workers for `downloads/v2-ahs-image-review/`, workers can report a saved PNG path even though no file is visible in the main workspace. Before marking a queue row `review`, future agents should `test -f` the candidate path, copy only verified files to `edited/`, and otherwise mark the row failed with a clear missing-artifact error.
- [2026-06-12] Codex-native image subagent concurrency capped at six: The multi-agent image batch hit `agent thread limit reached` above six active subagents. When draining this image queue through subagents, run at most six concurrent workers, close completed agents promptly, and reduce concurrency if many workers return preview-only/no-file results.
