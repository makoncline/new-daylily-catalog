...
- [2026-07-11] Offline full-app integration boundary: `pnpm exec tsx scripts/run-hermetic-local.ts` aliases Clerk only inside a guarded local process, uses purpose-specific personas on a fresh `tests/.tmp` SQLite DB, and blocks non-local browser traffic. Use `pnpm exec playwright test -c playwright.hermetic.config.ts` for real UI/tRPC/database happy paths; keep stage Clerk/Stripe verification in connected E2E, and never reuse a mutation persona for baseline-count assertions.
