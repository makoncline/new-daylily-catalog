...
- [2026-05-15] Public listing crawl cost shape: `/:userSlugOrId/:listingSlugOrId` uses replica-backed reads, and was moved off open-ended `force-static` ISR after bot crawls retained per-path Next cache/request objects. When investigating bot crawl cost on public listing pages, start at `apps/main/src/app/(public)/[userSlugOrId]/[listingSlugOrId]/page.tsx`, `src/proxy.ts`, `public-listing-read-model.ts`, and `public-seller-read-model.ts`; distinguish remote-DB latency from per-URL static generation, CDN/proxy cache headers, Prisma relation query fan-out, and client payload serialization.
