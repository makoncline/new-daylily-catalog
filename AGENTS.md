...
- [2026-07-10] E2E CI scaling boundary: The 14 local Playwright flows took 5.8 minutes on one CI worker, and they share one database/server within a Playwright process. Preserve every flow while reducing wall time by sharding files across separate CI jobs; each shard must retain its own generated temp database, Next server, and uniquely named artifacts. Do not enable multiple workers against one shared local database.
