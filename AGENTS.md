...
- [2026-07-11] Hermetic full-app boundary: `pnpm hermetic:dev` aliases Clerk only inside a guarded local process, uses purpose-specific personas on a fresh `tests/.tmp` SQLite DB, and blocks non-local browser traffic. Use `pnpm test:app` for real UI/tRPC/database happy paths; keep stage Clerk/Stripe verification in connected E2E, and never reuse a mutation persona for baseline-count assertions.
