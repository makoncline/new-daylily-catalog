...
- [2026-07-17] Importer cultivar facets need match payload fields: The catalog importer preview filters its already-matched browser rows rather than querying the cultivar search endpoint again. When adding a cultivar facet to the shared preview panel, also select and return its `CultivarSearchIndex` field from every exact, saved-ID, FTS, and fallback path in `cultivar-name-match.ts`.
- [2026-07-21] Cloudflare origin-header rules also honor Next headers: `bypass_by_default` follows ordinary `Cache-Control` when no CDN-specific header is present, so excluded force-static or ISR documents can still cache. When keeping a hostname-wide rule, send `Cloudflare-CDN-Cache-Control: no-store` for excluded static documents and prove them through the edge.
- [2026-07-21] Clerk session cookie names may be suffixed: Clerk accepts both `__session` and `__session_<suffix>` cookies. When guarding shared caches in the app or Cloudflare, match the `__session` name prefix rather than only the exact cookie name.
- [2026-07-20] Local production-shaped paths stay separate: `pnpm db:seed:prepare && pnpm dev` uses the sanitized realistic database, stage Clerk, Stripe test mode, and the `prodlike+` personas; `prod-like-local-docker-smoke.md` runs the actual production container with production service configuration and a local production DB copy. Do not use stage persona credentials to judge the production-container path or feed stage env into that Docker workflow.
- [2026-07-18] Full-app image upload boundary: The integration network guard intentionally forbids `AWS_BUCKET_NAME` and all R2 configuration, so listing-media integration can safely cover seeded local-image preview, reorder, and deletion without external traffic. When adding offline upload coverage, introduce an explicit loopback S3/R2 provider seam in a separate production-code PR; do not mock the app's tRPC procedures or enable real storage credentials.
- [2026-07-18] Atlas integration runner distinction: Files under `tests/integration/*.integration.ts` are Playwright full-app tests launched through `node apps/main/scripts/run-integration-local.mjs`; ordinary flow tests labeled integration run in Vitest. When adding flow confidence references, preserve separate runner metadata so Atlas never suggests Vitest for a full-app test.
- [2026-07-18] SES integration endpoint is explicit: The installed `@aws-sdk/client-ses` does not automatically apply `AWS_ENDPOINT_URL_SES` to `SESClient`; `public-inquiry.ts` passes it only when `INTEGRATION_MODE=1`. When testing app-generated email, keep placeholder AWS credentials and the loopback provider endpoint instead of adding SMTP or contacting AWS.
- [2026-07-17] Atlas lazy-image readiness: Waiting on `image.decode()` before scrolling a full page can deadlock on native lazy images that were never requested. Traverse the full capture geometry first, wait boundedly for scoped visible images, restore scroll, and retain the full-page height assertion; component captures must not wait on background-page images.
- [2026-07-17] Atlas expected document 404s: Chromium emits a generic console error for a legitimate main-document 404 that is indistinguishable from missing asset/API 404s by message alone. Use `gotoExpectedDocument404` so Atlas first asserts the navigation response is a document with status 404, then consumes exactly one matching diagnostic; never globally allow 404 console errors.
- [2026-07-17] Production snapshot checkout ownership: Local `db-backup.sh` restores `local-prod-copy-daylily-catalog.db` into the primary checkout derived from Git's common directory, even when invoked from a linked worktree. Use `pnpm db:seed:prepare` to create each worktree's sanitized realistic database; copy the full production snapshot into a worktree only for an exceptional manual workflow.
- [2026-07-17] Concurrent Atlas runs can select another port: `run-atlas-flow.mjs` defaults to port 3210, but another worktree may already own it. Set `ATLAS_PORT` to a free port so the runner still creates its disposable database and manages its own server; do not reuse or kill another worktree's Atlas server.
- [2026-07-17] Atlas seed existence is not freshness: `run-atlas-flow.mjs` compares the realistic seed manifest's schema fingerprint with the current Prisma schema and runs against a disposable database copy. If it reports a missing or stale seed, run `pnpm db:seed:prepare`; do not bypass the check or point Atlas at the canonical seed.
- [2026-07-16] Seeded Turso restore ordering: Piping a physical-order SQLite `.dump` into `turso db shell` can leave a mixed database when foreign-key parent tables are dropped or loaded out of dependency order, and the CLI can still exit successfully after rejected inserts. When changing `db:seed:sync`, keep child-first drops, parent-first inserts, and the exact post-sync table-count check.
- [2026-07-16] AHS Flower Show source discrepancy: WordPress REST `flower-show-section` taxonomy can disagree with both the rendered cultivar page and the `cultivar_search` payload; `A Different Kinda Buzz` reports `Seedling` through REST but `Large` on the page and in search results. When refreshing `flower_show`, trust the search payload and validate against rendered pages rather than the REST taxonomy.
- [2026-07-16] V2 AHS raw-key contract: Live cultivar search rows currently have 44 top-level keys; `awards` to `awards_json` and `first_image` to `image_url` are deliberate transforms, while the other 42 keys should be stored directly. When changing V2 ingestion, update `scripts/scrape/v2-ahs-source-field-contract.json` and its captured integration fixture so upstream additions or accidental omissions fail a focused test.
- [2026-07-16] Public cultivar exact facets need dedicated indexes: Applying `lower(trim(...))` to hybridizer rows or scanning pipe-delimited award names made exact `/cultivars` facets take hundreds of milliseconds on the 104k snapshot. Keep hybridizer filtering on indexed `hybridizerSearch`, store cultivar-to-award mappings in `CultivarSearchAward`, and benchmark filtered name-order queries whenever these facets change.
- [2026-07-16] Codex-native image backlog refill: Persist each queue-id-to-agent-id mapping before continuing, promote by that mapping rather than completion order, and allow non-linked backlog top-ups while other backlog rows are active. Only claimable linked-listing rows should block backlog additions.
- [2026-07-16] Docker Sentry release attribution: Production source maps, Node/edge events, and browser events must share the full GitHub commit SHA. Pass `github.sha` as the required Docker `GIT_COMMIT_SHA` build arg, expose it as `SENTRY_RELEASE` in both builder and runner stages, and send the runtime value through `/api/runtime-config` to the browser SDK.
- [2026-07-16] Standalone Sharp runtime validation: The production dependency deployment must merge into `/app/node_modules`, where the standalone server resolves external packages, rather than only `/app/apps/main/node_modules`. When validating the final runner, create a require from `/app/apps/main/server.js`, perform a tiny in-memory Sharp encode, find the loaded Sharp `.node` path in `require.cache`, and run `ldd` on that exact addon; PNPM keeps `@img/sharp-*` private, so direct top-level `require.resolve("@img/sharp-linux-x64")` is not a reliable check.
- [2026-07-16] Playwright screenshot caret hydration race: Playwright screenshots hide the caret by temporarily adding `style="caret-color: transparent"` to the focused element, which can race React hydration and produce a false hydration-mismatch error on file inputs. When taking an immediate screenshot of a Next page before hydration settles, pass `caret: "initial"` or wait for hydration before treating the console warning as a product bug.
- [2026-07-16] Generated cultivar ImageAsset flag retired: `USE_GENERATED_CULTIVAR_IMAGE_ASSETS` is baselined on after two stable production weeks. Generated cultivar reads should always query ready ImageAssets and preserve the legacy AHS image URL only as a missing-asset fallback.
...
- [2026-07-15] Public search listing limit zero is meaningful: Summary-mode searches pass `listingLimit: 0` to skip seller-listing sample expansion. Validate this option with an explicit `undefined` check; a falsy default converts zero back to the expensive default and silently defeats the summary hot path.
- [2026-07-15] Seeded production smoke native adapter: Next 16 webpack production builds must keep `@prisma/adapter-better-sqlite3` and `better-sqlite3` in `serverExternalPackages`; otherwise a seeded `next start` request fails to locate `better_sqlite3.node`. When benchmarking the production server against `apps/main/local/realistic-data/realistic-data.sqlite`, preserve that externalization.
- [2026-07-15] Next/OG Sharp version alignment: Next 16.3.0 loads Sharp 0.35.3; keeping the app's direct Sharp dependency on 0.34.5 loads two libvips versions and can produce black or corrupt tiles in Satori OG renders. Pin the app to Sharp 0.35.3 while this Next version is installed, and verify `pnpm why sharp` shows one version before testing ImageAsset WebP normalization.
- [2026-07-14] Seeded database command surface: `pnpm db:seed:prepare` generates the production-shaped development seed, ordinary `pnpm dev` uses it by default unless `DATABASE_URL` is explicitly set, and `pnpm db:seed:sync` uploads it to the seeded Turso database. Do not add separate worktree or seeded-dev launcher scripts; the same commands work in every checkout.
- [2026-07-14] PostHog survey white-labeling is unavailable: The connected project rejects survey appearance `whiteLabel=true` unless upgraded to Enterprise. When creating surveys through the PostHog connector, omit `whiteLabel` and use the default branding.
- [2026-07-13] Large-catalog Chrome persona ID can drift: The signed-in `prodlike+clerk_test_rollingoaks@example.com` Chrome session currently authenticates as Clerk user `user_3GK6izDToJuwZdf9C8zCPbtnBk5`, not the older `user_32T1tvQIoeDiev3SJwar7ogR8oo` documented for E2E. When cloning the production snapshot for visible large-catalog dashboard tests, verify the current Clerk ID from the clone after the first request and remap only the disposable clone instead of assuming the older ID.
- [2026-07-11] Read-only Sentry token in Keychain: A personal token with `event:read`, `org:read`, and `project:read` is stored in macOS Keychain as service `codex-sentry-readonly`, account `makon`. When running the bundled Sentry API helper, load it with `security find-generic-password -a makon -s codex-sentry-readonly -w` instead of storing the token in repo env files.
- [2026-07-10] Clerk `email.created` is an outbound email event: Its `data.id` identifies the email resource, not a user or email-address update. When filtering Clerk webhooks for user synchronization, ignore this event unless outbound-email behavior is explicitly needed; never treat its resource ID as a Clerk user ID.
- [2026-06-20] Daylily Codex-native image loop: The smooth V2 image-generation path is three fresh subagents max, each with one local image plus the exact square-output prompt; the main thread must track `queue id -> agent id`, promote from `~/.codex/generated_images/{agentId}` with `promote-codex-native-batch.mjs`, close completed agents, and only then refill to three. This matters when draining `/Users/makon/daylily-catalog-image-processing/v2-ahs-image-review/review.sqlite`; do not ask subagents to save candidates or update the DB themselves.
- [2026-07-16] Daylily Codex-native image worker: Drain bounded review batches with `run-codex-native-worker.mjs`; it records each `thread.started` session id, promotes only that session's PNG, and moves the row to `review` only after both local copies exist. Use the manual subagent flow only for custom one-offs.
- [2026-07-01] Vitest dynamic import.meta URL caveat: Vite can transform dynamic `new URL(\`../${sourcePath}\`, import.meta.url)`helpers in Vitest files and resolve them as`undefined`. When a filesystem test needs to read source files by dynamic path, use `process.cwd()`plus`node:path` instead.
- [2026-07-04] High-cardinality ISR breadcrumbs: `PublicBreadcrumbs` is route-aware client code (`usePathname` plus tRPC fallback hooks) and caused local `arrayBuffers` retention on `/{seller}/{listing}` under crawler-shaped traffic, while static server breadcrumbs recovered. When public ISR/SSG pages already have breadcrumb data server-side, render route chrome on the server and verify with the process in `apps/main/docs/local-issue-debugging-runbook.md`.
- [2026-07-05] Cloudflare SWR proof: Cloudflare treats `s-maxage` as blocking async stale-while-revalidate, returning `EXPIRED` instead of `UPDATING`. The initial `/catalogs` proof used a scoped Cache Response Rule to remove `s-maxage`, set `max-age`, and set `stale-while-revalidate`; for the current public HTML cache architecture, prefer the app-set `Cloudflare-CDN-Cache-Control` strategy documented in `apps/main/docs/public-cloudflare-html-cache.md`.
- [2026-07-05] Public CDN headers are pre-status: `src/proxy.ts` can add `Cloudflare-CDN-Cache-Control` before an App Router public page knows whether it will return 200 or 404. When using proxy-set CDN headers for dynamic public pages, keep a Cloudflare status-code guard so missing sellers/listings and 5xx responses do not inherit the successful-page TTL.
- [2026-07-05] Bare `_rsc` query is edge-only: In standalone `next start`, a request like `/{seller}?_rsc=probe` without component headers can still reach proxy code as a normal HTML document, while real component requests with `Accept: text/x-component` are no-stored. When validating Cloudflare public HTML caching, exclude `_rsc` in the Cloudflare rule and test origin RSC behavior with component request headers.
- [2026-07-09] Next 16 local route-handler host binding: Running `next dev --hostname 127.0.0.1` can leave route-handler requests such as `/api/runtime-config` self-proxying through `http://localhost:<port>` and failing with a socket hangup/500 even while page routes work. When locally testing this app with Next 16.2.6, bind the dev server to `localhost` and open the same `localhost` origin.
- [2026-07-05] zsh `path` loop variable breaks commands: `path` is a special zsh array tied to `PATH`; using `for path in ...` can make later commands report `command not found`. In shell probes, use variable names like `route_name` or call tools by absolute path if a loop mysteriously breaks command lookup.
- [2026-07-04] Disabled tRPC hooks still need context: `api.*.useQuery(..., { enabled: false })` still reads the tRPC React context during render. When adding optional public-page fetches, render no hook until the feature is active, or place the hook inside a child that is mounted under `TRPCReactProvider` only when needed.
- [2026-06-20] Onboarding local Chrome host: Anonymous onboarding middleware can rewrite requests to `http://localhost:<port>`, so a manual dev server bound only to `127.0.0.1` can produce socket hangups on `/onboarding`. When doing local Chrome/manual onboarding review, start Next on `localhost` or the default host and open `http://localhost:<port>`.
- [2026-06-18] Cloudflare R2 AWS SDK clients need path-style requests: In this repo, S3Client uploads to R2 failed DNS resolution when the SDK used virtual-hosted bucket names like `daylily-catalog-media.<account>.r2.cloudflarestorage.com`. Set `forcePathStyle: true` on R2 `S3Client` instances so requests target `<account>.r2.cloudflarestorage.com/<bucket>/...`.
- [2026-06-18] zsh status is read-only: Long-running shell wrappers in zsh can fail at the end with `read-only variable: status` if they assign `status=$?`. Use another variable name such as `exit_code` when wrapping scripts so successful work is not reported as a wrapper failure.
- [2026-05-15] Sentry production env can include previews: `environment=production` issue queries can return events from `*.deploy-preview.daylilycatalog.com` as well as `daylilycatalog.com`. When triaging production Sentry issues, inspect event `url`/host before treating every production-environment hit as live customer production.
- [2026-06-13] Fresh sibling worktree test setup: A clean sibling worktree under `/Users/makon/dev/` may not have its own `node_modules`. When targeted Node tests are needed without a fresh install, symlink the main checkout's root `node_modules`; expect Vite/Vitest to write transient `.vite-temp` files under that linked install, so rerun with approval if the sandbox blocks cache writes.
- [2026-06-14] Sibling worktree git metadata permissions: This Codex worktree stores git metadata under `/Users/makon/dev/new-daylily-catalog/.git/worktrees/new-daylily-catalog5`, outside the writable sandbox. When resetting or cleaning this worktree, rerun explicit git reset/clean commands with approval if index lock creation is blocked.
- [2026-05-15] Listings page E2E duration: `tests/e2e/listings-page-features.e2e.ts` is an intentionally broad multi-phase dashboard integration test and can approach the default 30s Playwright timeout on CI, especially when helpers simulate real keystrokes. Keep it marked `test.slow()` instead of weakening the user-interaction helpers or splitting the coverage casually.
- [2026-05-15] Dashboard replica refresh freshness caveat: Production logs showed dashboard primary chunk endpoints are slow while replica variants are subsecond, but the replica can lag by about 600s. Do not replace primary background reconciliation with replica refresh unless stale snapshot overwrite semantics are addressed; keep replica for first render/bootstrap and use primary or mutation-aware reconciliation for freshness.
- [2026-05-15] Live-vs-replica ownership split: Public pages and unchanging catalog data can use the embedded replica, but dashboard user-owned data must use the live primary because replica lag can hide or overwrite recent dashboard writes. When optimizing dashboard performance, do not move listing/list/image/user-owned reads to replica; instead optimize primary queries or change reconciliation semantics explicitly.
...
- [2026-08-29] Search index replica boundary: Production has one normal embedded Turso replica. Before an index rebuild, explicitly sync the app's normal replica client. Page source rows only through the singleton `replicaDb` connection and stream those bounded pages to a target-only child. The child must never open the replica. Never open or copy the libSQL-managed replica file with stock SQLite. Build and validate the local `.next` target, replace the serving index atomically, and keep `.previous` as the last-known-good index. `PUBLIC_SEARCH_INDEX_REFRESH_INTERVAL_SECONDS=0` stops rebuilds while a compatible serving index remains available.
...
- [2026-04-28, updated 2026-07-09] TanStack query-db pin baseline: The dashboard collection layer now runs `@tanstack/query-db-collection@1.x` with `@tanstack/query-core` pinned to `5.100.10` via the root pnpm override. Keep query-core and query-db-collection upgrades together, verify dashboard collection tests after any bump, and update the root override rather than removing it.
...
- [2026-04-08] Client-shared NEXT*PUBLIC flag helpers: Any module reachable from client components must not import `@/env` just to read a public feature flag. In this repo, read `process.env.NEXT_PUBLIC*\*`directly in shared/client-safe helpers or the production client will throw`Attempted to access a server-side environment variable on the client`.
...
- [2026-06-19] Stripe failed-payment customer emails enabled: In the Daylily Catalog Stripe dashboard, Billing -> Subscriptions can email customers when payment fails. When failed-payment notifications matter, rely on Stripe's built-in customer notification path before adding app-side owner alerts.
- [2026-06-26] Prod-like local Docker smoke: Use `apps/main/scripts/prepare-prod-like-local-smoke.mjs` to pull `/srv/stacks/daylilycatalog/.env`, point runtime at a mounted local SQLite DB, disable replica/search refresh, and route only `dev.daylilycatalog.com` in `~/.cloudflared/config.yml` to the local Docker port. This keeps production Clerk-domain testing repeatable without touching other tunnel ingress entries.
- [2026-06-29] V2 cultivar display flag retired: `NEXT_PUBLIC_USE_V2_CULTIVAR_DISPLAY_DATA` is baselined on and removed from app env/config/tests. When touching cultivar display reads, keep them V2-only, key dashboard AHS lookup by `CultivarReference.id`, and do not restore legacy `AhsListing` fallback branches.
- [2026-07-09] Worktree-local OpenAI key setup: This worktree's `.env.development` is a symlink, which the secure API-key writer correctly refuses; use ignored `.env.local` for a worktree-local key. For image moderation smoke tests, use a normal 512px image because OpenAI returned 500 for a 1px probe while the production-shaped 512px WebP request succeeded.
- [2026-07-15] Prisma client and stale Next dev cache: A worktree can keep a copied `.next/dev` external for `@prisma/client` that still fails on `.prisma/client/default` after `prisma generate`. When the generated client exists but Next dev still reports that module missing, stop Next and clear only that worktree's `apps/main/.next` before retrying.
- [2026-07-15] Contact-cart blur race: The contact dialog auto-focuses Email, and react-hook-form blur validation can rerender a cart row between pointer-down and click, swallowing the first quantity/remove action. Keep pointer focus on the row action group and verify these controls immediately after opening the dialog without warming up another field.
- [2026-07-12] ImageAsset read flag retired: `USE_IMAGE_ASSETS` is baselined on and removed from app env/config/tests. When touching listing, seller-profile, Merchant feed, onboarding, or dashboard image reads, keep ImageAsset relations unconditional while preserving legacy image URLs only as the missing-asset fallback.
- [2026-07-17] Flywheel Next development issue gate: Visible-browser verification is blocked while the tested flow reports any Next development issue. Open and inspect every issue, fix narrow tooling/test causes or split production app defects into a separate change, and never hide or crop out the issue badge.
- [2026-07-22] Cached public API link origins: Public search responses contain absolute catalog and listing URLs, so building them from request-supplied forwarded hosts can let a cacheable request prime incorrect links for other clients. When caching a public response with absolute links, use the trusted deployment base URL rather than request host headers.
