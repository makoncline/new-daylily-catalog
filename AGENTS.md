...
- [2026-04-27] Dashboard full bootstrap is listing-anchored: Cold bootstrap and warm full refresh should call `dashboardDb.bootstrap.snapshot`, fetch listings first, derive listing/profile/cultivar-reference ids, then fetch images, lists, and cultivar references by direct ids. When changing full user-data load behavior, keep this one snapshot path as the source for collection cache seeding and leave individual `*.sync` queries for incremental retries/mutations.
