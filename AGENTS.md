...
- [2026-07-11] Integration test port isolation: A long-lived exploratory app on IPv4 port 3200 can coexist with a Playwright-managed Next server bound to IPv6 on the same port, making health probes see the wrong process and eventually time out. Keep ordinary browser exploration on 3200, use the dedicated managed-test default in `playwright.integration.config.ts`, and attach explicitly with `BASE_URL` when testing an already-running app.
- [2026-07-11] Offline integration network enforcement: Browser request routing cannot detect server-side SDK traffic. The integration launcher preloads `scripts/integration-network-guard.mjs` only into the guarded Next process after database setup; keep its fail-closed runtime validation and loopback-only socket policy when adding local third-party adapters.
- [2026-07-11] Hermetic safety belongs in the app boundary: Launcher-only validation does not protect destructive local routes if `HERMETIC_MODE` is accidentally set elsewhere. Make every `isHermeticMode()` consumer validate non-production, local HTTP, `tests/.tmp` SQLite, and non-live credentials; identify reusable realistic-data servers through `/api/runtime-config` with their exact database path rather than accepting any healthy localhost process.
- [2026-07-11] Offline full-app integration boundary: `pnpm exec tsx scripts/run-integration-local.ts` aliases Clerk only inside a guarded local process, uses purpose-specific personas on a fresh `tests/.tmp` SQLite DB, and blocks non-local browser and server traffic. Use `pnpm exec playwright test -c playwright.integration.config.ts` for real UI/tRPC/database happy paths; keep stage Clerk/Stripe verification in connected E2E, and never reuse a mutation persona for baseline-count assertions.
- [2026-07-11] Prisma hermetic DB setup: The current Prisma CLI rejects the older `prisma db push --skip-generate` option. For fresh temporary SQLite databases, run plain `pnpm exec prisma db push` with the run-scoped `DATABASE_URL`.
...
- [2026-07-11] Preview smoke deployment recursion: Jobs declaring `environment: preview` create GitHub deployment-status records, so a `deployment_status` workflow that filters only by environment can trigger itself and every PR E2E job recursively. Require a successful status created by `vercel[bot]` with a non-empty `environment_url`; keep concurrency cancellation as a backup, not the primary filter.
- [2026-07-11] Related-test speed boundary: Vitest related selection cut a leaf `currency-input.tsx` change from the 26.47s full suite to 4.16s for two tests, but a central AHS display mapper selected 23 files and took 25.8s. Use related tests to add fast local signal for leaf changes, retain full CI, and do not claim a speedup for shared dependency changes.
- [2026-07-10] E2E CI scaling boundary: The 14 local Playwright flows took 5.8 minutes on one CI worker and 7m43s including setup, and they share one database/server within a Playwright process. Three isolated CI shards preserved all 14 flows and reduced the slowest complete job to 3m24-3m33s, including a warm run with two flaky retries. Keep each shard on its own generated temp database and Next server with uniquely named artifacts; do not enable multiple workers against one shared local database.
- [2026-07-10] E2E cache benchmark rejected: Preserving a separate Next E2E compiler cache reduced one unchanged test body from 8.0s to 5.2s, but total cold/warm run time regressed from 20.1s to 21.8s because database and server startup dominated. Do not keep E2E cache machinery based on compilation timing alone; benchmark complete command wall time and require a repeatable improvement first.
- [2026-07-10] Standalone Docker runtime dependency proof: PR #228 copied full root/app `node_modules`, but current Next automatic tracing still omits all five external server imports and pnpm app-relative tracing globs do not carry transitive symlink targets. Keep the lockfile-backed `packages/standalone-runtime` manifest limited to packages imported by current server code, deploy its production closure beside standalone, copy only the generated `.prisma` client from app build dependencies, and make the final runner `require()` every declared external before verifying `/` and `/catalogs`. This produced an 868 MB image versus 1.76 GB production.
- [2026-07-10] GitHub Docker cache mounts need explicit persistence: Docker's `type=gha` exporter does not preserve `RUN --mount=type=cache` contents across ephemeral Actions runners. For the source-change-heavy Next Docker build, persist `/app/apps/main/.next/cache` with `actions/cache` plus the commit-pinned `buildkit-cache-dance`; do not add package-manager cache mounts without the same persistence path and measured benefit.
- [2026-07-10] Docker PR Sentry boundary: Pull-request Docker images are verification-only (`push: false`) and never deploy, so do not provide `SENTRY_AUTH_TOKEN` or run Sentry sourcemap processing for them. Keep production main-image sourcemap behavior unchanged.
- [2026-07-10] Bound optional agent builds: Local Next 16 Turbopack production builds can become fully idle during compilation without exiting or emitting an error. Keep `agent:build` opt-in and bound its wrapper to 180 seconds with `AGENT_BUILD_TIMEOUT_SECONDS` override so a stalled compiler cannot trap the entire verification loop; do not apply this timeout to deployment builds.
- [2026-07-10] Atlas comparison encoding cost: Writing full-page diff PNGs for unchanged screenshots dominated clean comparison time. Always calculate selected-image pixel differences, but only encode diff assets when the percentage crosses the review threshold; keep targeted story comparisons scoped with `--stories` while leaving the full gallery intact.
- [2026-07-10] Agent loop startup and output: Atlas orchestration only needs a responsive Next server, so health-check `/api/runtime-config` instead of compiling the data-heavy `/catalogs` route, and start owned realistic-data servers with `LOCAL_QUERY_LOGGING=0` to prevent large-dashboard Prisma query text from burying Playwright results. Keep normal local query logging enabled outside the agent-owned loop.
- [2026-07-10] TypeScript 7 agent typecheck bridge: Stable TypeScript 7 cuts this app's full typecheck from about 9.5s to 1.2s, but `typescript-eslint@8.56` crashes when the installed `typescript` package is 7.x because it requires the pre-6 compiler API. Keep TypeScript 5.9 installed for ESLint/Next compatibility, pin `@typescript/native-preview` side-by-side for `tsgo`, use `typecheck:legacy` to compare results, and only replace the main package after typescript-eslint supports TS7.
- [2026-07-10] Next 16 allows one dev server per app directory: A second `next dev` from the same `apps/main` directory exits even on a different port and reports the existing PID because `.next/dev` is locked. When verifying owned-server lifecycle, stop the existing app first or use another checkout; orchestration should detect early child exit instead of waiting out its health timeout.
- [2026-07-10] Agent atlas screenshot stability: Onboarding starter images and canvas-derived previews can differ if captured immediately, while some remote images never settle. Before atlas screenshots, emulate reduced motion, await fonts, wait for images with a bounded 2s race, then allow a short render settle; never put an unbounded image wait or per-checkpoint `networkidle` in the shared capture helper.
- [2026-07-10] Agent atlas dashboard navigation: Large realistic-data dashboards keep background cultivar-reference requests active after the heading renders; navigating to the next checkpoint immediately produces false `TRPCClientError: Failed to fetch` console failures. In authenticated atlas flows, wait for `networkidle` before capturing and moving to the next dashboard route, and allow a 120s test timeout for large catalogs.
- [2026-07-10] Realistic-data public catalog visibility: `/catalogs` filters users through cached Stripe subscription status. Preserve every copied production `active`/`trialing` state under a synthetic local-only Stripe customer ID, while only selected personas receive stage Clerk and real Stripe test identities; this keeps the directory representative without enabling login or contacting live Stripe.
- [2026-07-10] Realistic-data and prod-like Docker are separate: `prepare-prod-like-local-smoke.mjs` intentionally runs the production Docker image locally with VPS-derived configuration, while `generate-realistic-data-snapshot.mjs` powers ordinary hot-reload development using a copied database plus stage Clerk and Stripe-test identities. Do not merge their docs, commands, env handling, or safety assumptions.
- [2026-07-10] Production smoke-account containment: An authenticated user without an active Stripe subscription can still use the dashboard, is omitted from `/catalogs` and sitemap generation, and has direct public profile/listing pages marked `noindex`. For agent production smoke tests, prefer one permanent synthetic live-Clerk account with no subscription, fake content, serialized runs, and run-owned cleanup; keep public read smoke on stable baseline rows because public replica/cache lag makes immediate mutation-to-public assertions unreliable.
- [2026-07-10] Realistic dev DB must stay separate from deterministic seeds: The local production copy contains private listing notes plus real Clerk, Stripe, and key-value state, while local E2E creates protected databases under `tests/.tmp` and preview smoke depends on the stable `seeded-daylily` catalog. When making development look production-like, clone the prod copy into an ignored output, sanitize only the output, remap one chosen catalog to the Clerk test user, and leave the existing test/preview seed path deterministic.
- [2026-07-10] Agent UI flywheel baseline: The app has 27 page routes and 16 broad Playwright E2E specs, but `playwright.config.ts` only keeps screenshots on failure and does not enable traces or videos; only `create-edit-listing-flow.e2e.ts` attaches intentional checkpoint screenshots. When building agent-facing visual coverage, add a separate deterministic page/feature-state atlas that reuses the existing fixtures and page objects instead of inflating the broad integration specs.
