...
- [2026-05-14] GSC token fallback: This worktree's `.env.development` exposed `GSC_ACCESS_TOKEN` and `GSC_REFRESH_TOKEN`, but the access token returned `invalid_token` and the refresh token returned `invalid_grant`; authenticated Chrome still had Search Console access. When GSC API calls fail here, switch the property in Chrome and record UI-visible data rather than assuming no Search Console access.
- [2026-05-14] Image cropper output size: The dashboard `ImageCropper` crops at source-image resolution unless explicitly capped, and server upload presign still enforces `APP_CONFIG.UPLOAD.MAX_FILE_SIZE` at 10MB. When debugging upload-size Sentry errors, check both the dropzone pre-crop file limit and the post-crop blob size before assuming the cropper resized/compressed the image.
- [2026-05-14] Dev tunnel port owner: `~/.cloudflared/config.yml` maps `dev.daylilycatalog.com` to `http://127.0.0.1:3002`, and OrbStack can keep an old Docker app bound there as `tmp-app-1` (`0.0.0.0:3002->3000`). When the dev tunnel shows stale code, check `docker ps`/`lsof -iTCP:3002` and stop the stale container before starting `PORT=3002 pnpm dev`.
- [2026-05-14] zsh `path` variable hazard: In zsh, `path` is tied to `PATH`; using `for path in ...` can clobber command lookup and make ordinary commands like `curl`, `rg`, and `sed` vanish in the same shell. Use names like `page_path` for loop variables.
- [2026-05-14] OG image transform cost guard: For Daylily-owned S3 listing images, social metadata should reuse the existing full-size app transform shape `width=800,fit=cover,format=auto,quality=90` through `cf.daylilycatalog.com` instead of adding new 1200x630 variants. This avoids extra Cloudflare transformation variants while also fixing old S3 objects with bad `Content-Type` metadata.
