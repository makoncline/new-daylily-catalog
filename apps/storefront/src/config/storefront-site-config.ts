...
type StorefrontSiteDefinition = Omit<
...
function getSiteDefinition() {
  ...
  selectedSite ??= storefrontSites["rolling-oaks"];
  if (
    requestedHostname &&
    !selectedSite.hostnames.includes(requestedHostname)
  ) {
    throw new Error(
      `STOREFRONT_HOSTNAME does not match the approved host for ${selectedSite.key}.`,
    );
  }
...
export function getStorefrontSiteConfig(): StorefrontSiteConfig {
  ...
  if (
    ...
    throw new Error(
      `STOREFRONT_SELLER_ID does not match the approved seller for ${site.key}.`,
    );
  }
