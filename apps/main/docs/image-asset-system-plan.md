...
to R2 and create the `ImageAsset` row. In v1, variants are generated by running
the manual processor script after upload/backfill.

Do not add a cron/systemd scheduled runner in v1. The worker script remains the
shared implementation for pending-row processing and manual repair, but the
first production rollout should avoid scheduled infrastructure until it is
actually needed.
...
- Do not actively revalidate public pages in v1. Let normal ISR/cache windows
  expire and pick up the new ImageAsset URLs.
- Set failed rows to `status = "variant_failed"` and exclude them from normal
  runs. Manual repair can opt in with `--retry-failed`.
- Emit enough stderr/stdout context to recover failed rows manually.
...
- Always log missing variant rows with the `ImageAsset.id`, owner fields, and
  missing keys.
