...
- Docker builds persist the environment-scoped Next compiler cache across GitHub-hosted runners. The normal `type=gha` layer cache remains enabled, while the cache-mount contents use Docker's documented cache-dance workaround because the GHA exporter does not preserve them directly.
