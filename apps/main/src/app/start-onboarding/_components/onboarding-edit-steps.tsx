...
interface OnboardingProfileEditStepFlags {
...
interface OnboardingListingEditStepFlags {
...
interface OnboardingProfileEditStepProps {
  ...
  flags: OnboardingProfileEditStepFlags;
...
export function OnboardingProfileEditStep(
  props: OnboardingProfileEditStepProps,
) {
  return (
    <div className="space-y-6">
      ...
      <OnboardingStepGrid className="lg:grid-cols-[minmax(0,1fr)_minmax(0,24rem)]">
        <OnboardingProfileEditorFields props={props} />
        <OnboardingProfilePreviewPanel props={props} />
      </OnboardingStepGrid>

      <OnboardingChecklist items={props.profileContinueChecklist} />
    </div>
  );
}

function OnboardingProfileEditorFields({
  props,
}: {
  props: OnboardingProfileEditStepProps;
}) {
    ...
    <div className="order-2 space-y-4 lg:order-1">
      <div
        className={cn(
          ...
          focusedProfileField === "gardenName" && "border-primary bg-primary/5",
        )}
      >
        ...
        <p className="text-muted-foreground text-xs leading-relaxed">
          This is your storefront title. Buyers use it to recognize and return
          to your catalog.
        </p>
      </div>

      <OnboardingProfileImageField props={props} />
...
function OnboardingProfileImageField({
  props,
}: {
  props: OnboardingProfileEditStepProps;
}) {
    ...
    >
      ...
      {existingProfileImageUrl ? (
        <div className="flex items-start gap-3 rounded-lg border border-dashed p-3">
          ...
          <div className="space-y-1 text-sm">
            ...
            <p className="text-muted-foreground text-xs leading-relaxed">
              Keep your current uploaded profile image unless you uncheck this
      ...
      {!useExistingProfileImage ? (
        <div className="grid gap-2 lg:grid-cols-2">
          ...
          <Button
            type="button"
            variant={profileImageInputMode === "upload" ? "default" : "outline"}
            onClick={() => {
              setFocusedProfileField("image");
...
function OnboardingProfilePreviewPanel({
  props,
}: {
  props: OnboardingProfileEditStepProps;
}) {
...
function OnboardingChecklist({ items }: { items: readonly ChecklistItem[] }) {
  return (
    <div className="space-y-2">
...
interface OnboardingListingEditStepProps {
  ...
  flags: OnboardingListingEditStepFlags;
...
export function OnboardingListingEditStep(
  props: OnboardingListingEditStepProps,
) {
  return (
    <div className="space-y-6">
      <div className="space-y-2">
      ...
      <OnboardingStepGrid className="lg:grid-cols-[minmax(0,1fr)_minmax(0,24rem)]">
        <OnboardingListingEditorFields props={props} />
        <OnboardingListingPreviewPanel props={props} />
      </OnboardingStepGrid>

      <OnboardingChecklist items={props.listingContinueChecklist} />
    </div>
  );
}

function OnboardingListingEditorFields({
  props,
}: {
  props: OnboardingListingEditStepProps;
}) {
  ...
  return (
    <div className="order-2 space-y-4 lg:order-1">
      <div
        ...
        onKeyDown={(event) => {
          if (event.key === "Enter" || event.key === " ") {
            setActiveListingField("cultivar");
          }
        }}
      >
        ...
        />
        <p className="text-muted-foreground text-xs leading-relaxed">
          We start you with a linked starter variety by default so your preview
          looks realistic. You can click to change it now, and create unlinked
        ...
        {selectedCultivarName ? (
          <div className="rounded-lg border border-emerald-500/40 bg-emerald-500/10 p-3">
            ...
            <p className="text-muted-foreground mt-3 text-xs leading-relaxed">
              You&apos;re viewing the Daylily Database reference card for this
              variety. The database includes details and images for 100,000+
              listings.
            </p>
            {selectedCultivarDetails ? (
        ...
        <p className="text-muted-foreground text-xs leading-relaxed">
          Linking a variety gives buyers trusted cultivar details and imagery.
        </p>
      </div>
      ...
      <div
        className={cn(
          "space-y-2 rounded-lg border p-4 transition-colors",
          activeListingField === "description" && "border-primary bg-primary/5",
        )}
      >
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
          ...
          >
            {listingDescriptionCharacterCount === 0
              ? getDescriptionLengthAimText({
                  minLength: ONBOARDING_LISTING_DESCRIPTION_GUIDANCE.minLength,
                  maxLength: ONBOARDING_LISTING_DESCRIPTION_GUIDANCE.maxLength,
                })
              : isListingDescriptionTooShort
...
function OnboardingListingPreviewPanel({
  props,
}: {
  props: OnboardingListingEditStepProps;
}) {
