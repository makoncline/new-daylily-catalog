...
let failedBrowserRequests: { errorText: string; url: string }[] = [];

function isAbortedReadError(message: string) {
  const matchingAbortedRequest = (path: string) =>
    failedBrowserRequests.some(
      (request) =>
        request.errorText.includes("ERR_ABORTED") && request.url.includes(path),
    );

  if (message.startsWith("Observability initialization failed:")) {
    return matchingAbortedRequest("/api/runtime-config");
  }
...
test.beforeEach(async ({ context, page, request }) => {
  ...
  await context.routeWebSocket(/.*/, (webSocket) => {
    ...
    networkLedger.push(`WS ${url.href}`);
    if (!LOCAL_HOSTS.has(url.hostname)) {
      throw new Error(`Hermetic test blocked outbound WebSocket: ${url.href}`);
    }
...
test.afterEach(async ({}, testInfo) => {
  ...
  await testInfo.attach("failed-browser-requests.json", {
    body: Buffer.from(JSON.stringify(failedBrowserRequests, null, 2)),
    contentType: "application/json",
  });
  expect(
    browserErrors.filter((message) => !isAbortedReadError(message)),
...
test("guest browses catalogs, a seller, a listing, and catalog search", async ({
  ...
  await expect
    ...
    .toBe(2);
  await page.goto("/local/email");
  await expect(
    page.getByRole("heading", { name: "Email inbox" }),
  ).toBeVisible();
  await expect(
    page.getByText("buyer@example.com", { exact: true }),
  ).toBeVisible();
  await expect(
    page
      .locator("pre")
      .filter({ hasText: "Is this daylily available for local pickup?" }),
  ).toHaveCount(2);
...
test("protected routes and a second persona use the local auth shell", async ({
  ...
  await new DashboardHome(page).waitForLoaded();
...
test("membership states use the local Stripe checkout and portal", async ({
  ...
  expect((await portalResponse).ok()).toBe(true);
  ...
  await Promise.all([
    page.waitForURL(/\/hermetic\/stripe\/checkout\?session_id=/, {
      timeout: 15_000,
    }),
    dashboard.upgradeToProButton.click(),
  ]);
  await expect(
    page.getByRole("heading", { name: "Test checkout" }),
  ).toBeVisible();
  await page.goto("/dashboard");
  await dashboard.waitForLoaded();
  await expect(dashboard.proMembershipCard).toBeVisible();
  ...
  await context.clearCookies();
  await page.goto("/sign-in");
  await page.locator('[data-hermetic-persona="free-at-limit"]').click();
...
test("seller uses listing filters, pagination, and list membership", async ({
  ...
  await dashboardLists.saveChangesAndWait();
