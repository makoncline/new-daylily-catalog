...
describe("generateRealisticDataSnapshot", () => {
  ...
  test("resets only Atlas-generated events and inquiry limits", () => {
    const directory = mkdtempSync(
      path.join(tmpdir(), "realistic-data-atlas-state-"),
    );
    const databasePath = path.join(directory, "realistic.sqlite");
    createSourceDatabase(databasePath);
    const db = new DatabaseSync(databasePath);
    db.exec(`
      INSERT INTO KeyValue (key, value) VALUES
        ('public-inquiry:one', '{}'),
        ('hermetic:event:email:one', '{}');
    `);
    db.close();

    expect(resetRealisticDataAtlasState(databasePath)).toBe(2);

    const output = new DatabaseSync(databasePath, { readOnly: true });
    expect(
      output.prepare("SELECT key FROM KeyValue ORDER BY key").all(),
    ).toEqual([
      { key: "clerk:user:prod-clerk-large" },
      { key: "feature:use-v2" },
      { key: "stripe:customer:prod-stripe-large" },
      { key: "stripe:customer:prod-stripe-unmapped" },
    ]);
    output.close();
  });
});
