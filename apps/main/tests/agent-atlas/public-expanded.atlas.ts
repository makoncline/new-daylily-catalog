...
const publicRoutes = [
  ["home-page", "/", "The daylily you want. From the grower who has it.", "Public home and catalog discovery entry point."],
  ["start-membership", "/start-membership", "Your whole daylily catalog. One link buyers can browse.", "Grower acquisition page with benefits, examples, pricing, and FAQ."],
...
test("catalog search results", async ({ page }, testInfo) => {
  ...
  await captureCheckpoint(page, testInfo, "catalog-search-results", "Public catalog search with an active query and filtered listing results.");
});
