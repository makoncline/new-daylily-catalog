...
test.describe("dashboard setup journey @local", () => {
  test("signed-in user can complete dashboard setup after sign-in", async ({
    ...
    await withTempE2EDb(async (db) => {
      ...
      // The dashboard entry point is sign-in only. New account creation must
      // happen through the post-checkout onboarding success screen.
      ...
      const profileSaveRequest = page.waitForResponse(
        (response) => {
          ...
          return (
            url.includes("dashboardDb.userProfile.update") &&
            !url.includes("dashboardDb.userProfile.updateContent")
          );
        },
        { timeout: 15000 },
      );
