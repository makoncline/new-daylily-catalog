...
if (agentId && currentRow.codexNativeAgentId !== agentId) {
  throw new Error(
    currentRow.codexNativeAgentId
      ? `Queue row ${id} is assigned to ${currentRow.codexNativeAgentId}, not ${agentId}`
      : `Queue row ${id} has no recorded Codex agent`,
  );
}
