...
const REVIEW_DB_PATH = path.join(
  DATA_ROOT,
  "v2-ahs-image-review",
  "review.sqlite",
);
const DEFAULT_OUT_DIR = path.join(DATA_ROOT, "v2-ahs-images");
const CONCURRENCY = 3;
const RETRIES = 3;
...
function loadImageRows(limit: number | null): ImageRow[] {
  const database = new DatabaseSync(PROD_DB_PATH, {
    open: true,
    readOnly: true,
  });
  const sourceInvalidIds = readSourceInvalidIds();
  ...
  try {
    const query = `
      SELECT
        ...
        COALESCE(NULLIF(v2."image_url", ''), NULLIF(ahs."ahsImageUrl", '')) AS "image_url",
        CASE
...
function readSourceInvalidIds(): Set<string> {
...
function mapFilesByStem(dirPath: string): Map<string, string> {
...
async function main() {
  ...
  let failed = 0;
  let nextIndex = 0;
  const existingOriginals = mapFilesByStem(DEFAULT_OUT_DIR);

  console.log(
    `[v2-images] linked-missing-generated queue=${rows.length} concurrency=${CONCURRENCY} db=${PROD_DB_PATH} out=${DEFAULT_OUT_DIR}`,
  );
  ...
  if (args.dryRun) {
    ...
    console.log(
      `[v2-images] dry-run v2=${sourceCounts.v2} legacy=${sourceCounts.legacy}`,
    );
    console.log(
      JSON.stringify(
  ...
  async function worker(workerId: number) {
    while (true) {
      ...
      if (existingOriginals.has(row.id) || fs.existsSync(outputPath)) {
        skipped += 1;
        continue;
      }
      ...
      if (lastError) {
        ...
        console.warn(
          `[v2-images] failed id=${row.id} source=${row.source_kind} title=${row.post_title ?? ""} error=${message}`,
        );
      }
    }
  }
  ...
  if (failures.length > 0) {
    fs.writeFileSync(
      failurePath,
      [
        "cultivarReferenceId\tpost_title\tsource_kind\timage_url\terror",
        ...failures,
      ].join("\n") + "\n",
      "utf8",
    );
