...
export function resetRealisticDataAtlasState(databasePath) {
  ...
  try {
    const result = db
      .prepare(
        ...
        WHERE key LIKE 'public-inquiry:%'
           OR key LIKE 'hermetic:event:%'
      `,
      )
      .run();
...
export async function generateRealisticDataSnapshot({
  ...
  try {
    ...
    db.exec(`
      ...
      DELETE FROM KeyValue
      WHERE key LIKE 'clerk:user:%' OR key LIKE 'stripe:customer:%';
    `);
