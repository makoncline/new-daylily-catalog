...
} from "./agent-atlas-baseline-state.mjs";
...
const selectedCaptureNames = parseComparisonCaptureNames(process.argv.slice(2));
const selectedProjects = parseComparisonProjects(process.argv.slice(2));
const runStartedAt = Number(process.env.AGENT_ATLAS_RUN_STARTED_AT ?? "0");
const isFullCapture =
  selectedStories === null &&
  selectedCaptureNames === null &&
  selectedProjects === null;

const baselineInitialized = initializeAtlasBaseline(captures, baseline, {
  fullCapture: isFullCapture,
});
...
const baselineApproved = isAtlasBaselineApproved(baseline);
...
const captureFiles = readdirSync(captures)
  .filter((name) => name.endsWith(".json"))
  .sort();
const currentRunItems = captureFiles
  .filter(
    (file) =>
      runStartedAt === 0 ||
      statSync(path.join(captures, file)).mtimeMs >= runStartedAt,
  )
  .map((file) => JSON.parse(readFileSync(path.join(captures, file), "utf8")));
const results = [];
for (const item of currentRunItems) {
  ...
  if (!baselineApproved || !existsSync(beforePath)) {
...
if (runStartedAt > 0) {
  const baselineItems = readdirSync(baseline)
    .filter((name) => name.endsWith(".json") && name !== "baseline.json")
    .map((file) => JSON.parse(readFileSync(path.join(baseline, file), "utf8")));
  for (const item of removedBaselineItems(
    baselineItems,
    currentRunItems,
    selectedStories,
    selectedCaptureNames,
    ...
    const beforeAsset = `${item.key}-before.png`;
    copyFileSync(
      path.join(baseline, `${item.key}.png`),
      path.join(assetRoot, beforeAsset),
    );
    results.push({
      ...item,
      status: "removed",
...
const cards = results
  .map((item) => {
    ...
    return `<article class="card ${item.status}"><header><strong>${escapeHtml(item.name)}</strong><span>${item.status} · ${item.percent.toFixed(3)}%</span></header><p>${escapeHtml(item.description ?? item.url)}</p><p class="meta">${escapeHtml(item.project)} · ${escapeHtml(item.story ?? "unknown story")} · ${escapeHtml(new URL(item.url).pathname)} · ${escapeHtml(item.viewport ? `${item.viewport.width}×${item.viewport.height}` : "viewport unknown")}</p>${diagnosticLines.length ? `<details open><summary>${diagnosticLines.length} browser diagnostics</summary><ul>${diagnosticLines.map((line) => `<li>${escapeHtml(line)}</li>`).join("")}</ul></details>` : ""}<p><strong>Rerun:</strong> <code>${escapeHtml(item.rerunCommand ?? "node scripts/run-agent-atlas-full.mjs")}</code></p><div class="images">${item.beforeAsset ? `<figure><figcaption>Baseline</figcaption><img src="comparison-assets/${item.beforeAsset}"></figure>` : ""}${item.afterAsset ? `<figure><figcaption>Current</figcaption><img src="comparison-assets/${item.afterAsset}"></figure>` : ""}${item.diffAsset ? `<figure><figcaption>Difference</figcaption><img src="comparison-assets/${item.diffAsset}"></figure>` : ""}</div></article>`;
  })
  .join("\n");
writeFileSync(
  path.join(reportRoot, "visual-review.html"),
  `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Atlas comparison</title><style>body{font:15px system-ui;margin:0;padding:32px;background:#f5f5f2;color:#20231f}main{display:grid;gap:20px}.summary,.card{background:white;border:1px solid #ddd;border-radius:14px;padding:18px}.notice{background:#fff7d6;border:1px solid #d97706;border-radius:8px;padding:10px}.card.review,.card.removed{border-color:#ef4444}.card.new{border-color:#d97706}header{display:flex;justify-content:space-between;gap:12px}.meta{color:#62685f}code{background:#f0f2ed;padding:4px 7px;border-radius:5px}.images{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}figure{margin:0;max-height:440px;overflow:auto;border:1px solid #ddd}figcaption{position:sticky;top:0;background:#20231f;color:white;padding:6px 9px}img{display:block;width:100%}@media(max-width:800px){.images{grid-template-columns:1fr}}</style></head><body><section class="summary"><h1>Atlas comparison</h1>${!baselineApproved ? '<p class="notice">This local reference is unapproved. Comparisons remain pending until you explicitly approve it.</p>' : ""}<p>${summary.unchanged} unchanged · ${summary.review} need review · ${summary.new} new · ${summary.removed} removed · ${summary.compared} compared</p><p>Review changed cards, inspect browser diagnostics, then rerun the shown command or approve intentional changes with <code>node scripts/agent-atlas-baseline.mjs</code>.</p></section><main>${cards}</main></body></html>`,
);
console.log(JSON.stringify(summary, null, 2));
if (
  (summary.review > 0 || summary.new > 0 || summary.removed > 0) &&
