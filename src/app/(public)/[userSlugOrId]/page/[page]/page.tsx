...
export async function generateMetadata({
  ...
  const profileResult = await tryCatch(getPublicProfilePageData(userSlugOrId, parsedPage));
  ...
  const baseMetadata = await generateProfileMetadata(profileResult.data.profile, baseUrl);
  const canonicalUserSlug = profileResult.data.profile.slug ?? profileResult.data.profile.id;
        ...
        <div className="space-y-6">
          ...
          <Suspense>
            <CatalogSeoListings
              ...
              searchQueryString={pageDataSearchParams.toString()}
            />
          </Suspense>
        </div>
      </MainContent>
