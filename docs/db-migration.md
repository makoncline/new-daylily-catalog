...
1. Structural changes: Prisma-generated SQL in `apps/web/prisma/migrations/...`.
2. Data changes: deterministic SQL files generated by script into `apps/web/prisma/data-migrations/...`.
...
- Do not use `pnpm web env:dev -- ...` (this causes `spawn -- ENOENT`).
- Always run env wrapper commands as `pnpm web env:dev <command>`.
...
```text
apps/web/prisma/local-prod-copy-daylily-catalog.db
```
...
npx prisma migrate diff \
  --from-url file:./apps/web/prisma/local-prod-copy-daylily-catalog.db \
  --to-schema-datamodel apps/web/prisma/schema.prisma \
  --script > apps/web/prisma/migrations/<timestamp>_<migration_name>/migration.sql
```

## Step 2: Generate Data SQL

1. Create/update a generator script in `apps/web/scripts/`.
2. Generate SQL into `apps/web/prisma/data-migrations/`.
...
```bash
sqlite3 apps/web/prisma/local-prod-copy-daylily-catalog.db < apps/web/prisma/migrations/<timestamp>_<migration_name>/migration.sql
sqlite3 apps/web/prisma/local-prod-copy-daylily-catalog.db < apps/web/prisma/data-migrations/<first_data_sql>.sql
sqlite3 apps/web/prisma/local-prod-copy-daylily-catalog.db < apps/web/prisma/data-migrations/<second_data_sql>.sql
```
...
```bash
turso db shell daylily-catalog-stage < apps/web/prisma/migrations/<timestamp>_<migration_name>/migration.sql
turso db shell daylily-catalog-stage < apps/web/prisma/data-migrations/<first_data_sql>.sql
turso db shell daylily-catalog-stage < apps/web/prisma/data-migrations/<second_data_sql>.sql
```
...
```bash
turso db shell daylily-catalog < apps/web/prisma/migrations/<timestamp>_<migration_name>/migration.sql
turso db shell daylily-catalog < apps/web/prisma/data-migrations/<first_data_sql>.sql
turso db shell daylily-catalog < apps/web/prisma/data-migrations/<second_data_sql>.sql
```
...
- `apps/web/prisma/schema.prisma` changes.
- Structural migration SQL under `apps/web/prisma/migrations/`.
- Data SQL generator script under `apps/web/scripts/`.
- Generated data SQL under `apps/web/prisma/data-migrations/`.
