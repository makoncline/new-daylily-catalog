...
export class OnboardingFlowPage {
  ...
  async expectStep(stepNumber: number, title: string) {
    await expect(
      this.container.getByText(new RegExp(`^${escapeRegExp(title)}$`)),
    ).toBeVisible({ timeout: 15000 });
    await expect(
      this.container.getByText(new RegExp(`^Step ${stepNumber} of \\d+$`)),
    ).toBeVisible({ timeout: 15000 });
  }
  ...
  async expectListingPreview({
    ...
    await expect(
      this.page.getByText(`$${price}.00`, { exact: false }).first(),
    ).toBeVisible();
    await expect(
      this.page.getByText(description, { exact: false }).first(),
    ).toBeVisible();
